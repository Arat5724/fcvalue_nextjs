...
export default function Page() {
  ...
  const [players, setPlayers] = useState<SeasonList>([]);
  const [seasonStates, setSeasonStates] = useState<boolean[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("https://fcvalue.com/players.json")
      .then(res => res.json())
      .then((data: SeasonList) => {
        setPlayers(data);
        setSeasonStates(new Array<boolean>(data.length).fill(false));
      });
  }, []);
...
function SeasonButtons({ seasonList, seasonStates, setSeasonStates, seasonStateSum, setSeasonStateSum }:
  ...
  function setSeasonState(index: number) {
    return () => {
      ...
      console.log(seasonStateSum + (newSeasonState[index] ? 1 : -1));
...
function UpgradeSimulator({ player }: {
  ...
  return <div className={styles["simulator-section"]}>
    ...
    <div className={styles["simulator-section__information"]}>
      <p>
        <span>강화 확률 </span>
        <span className={styles.primary}>{(upgradeProb[upgrade] * blockState * 20).toFixed(2)}</span>
        <span>%</span>
      </p>
      <p>
    ...
    {result === UpgradeResult.No
      ? <button className={styles["button__upgrade"]} onClick={upgradePlayer}
        disabled={player === undefined}>강화 시도</button>
      : <button className={styles["button__retry"]} onClick={resetPlayer}
        disabled={player === undefined}>다시 시도</button>}
  </div>
}
